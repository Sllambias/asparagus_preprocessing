# Custom version of CLS002_FOMO26_Infarct.py that supports different modalities.
...
TASK1_MODALITY_FILES = {
    "flair": "flair.nii.gz",
    "adc": "adc.nii.gz",
    "dwi": "dwi_b1000.nii.gz",
    "swi": "swi.nii.gz",
}
DEFAULT_MODALITIES = ["flair", "adc", "dwi"]
...
def normalize_modalities(modalities=None):
    ...
    if not modalities:
        raise ValueError("At least one Task 1 modality must be selected.")
    unknown = sorted(set(modalities) - set(TASK1_MODALITY_FILES))
    if unknown:
        raise ValueError(f"Unknown Task 1 modalities: {unknown}")
    return modalities


def modality_data_path(file_out, modalities):
    if modalities == DEFAULT_MODALITIES or modalities == ["flair"]:
        return file_out
    return os.path.join(os.path.dirname(file_out), "_".join(modalities))
...
def process_sample(file_in, file_out, dataset_config, preprocessing_config, saving_config):
    ...
    file_out = modality_data_path(file_out, modalities)
    ...
    required = [os.path.join(case_dir, TASK1_MODALITY_FILES[modality]) for modality in modalities]
    ...
    images = [nib.load(path) for path in required]
...
def main(
    ...
    task_name: str = "CLS002_FOMO26_Infarct_CUSTOM",
    modalities=None,
):
    modalities = normalize_modalities(modalities)
    if task_name == "CLS002_FOMO26_Infarct" and modalities != DEFAULT_MODALITIES:
        raise ValueError("Use a new task_name when changing Task 1 modalities.")
