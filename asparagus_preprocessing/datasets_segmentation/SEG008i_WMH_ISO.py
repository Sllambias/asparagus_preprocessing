...
from asparagus_preprocessing.utils.saving import save_data_and_metadata, save_modified_label
...
def process_sample(file_in, file_out, dataset_config, preprocess_config, saving_config):
    ...
    # assert that case has label map
    if not os.path.exists(file_in.replace("T1", "wmh")):
        logging.info(f"skipping {file_in}, no label map found")
        return

    # Remove city name
    for city in ["Amsterdam", "Singapore", "Utrecht"]:
        file_out = file_out.replace(city + os.sep, "")

    # Rename weird training folder
    file_out = file_out.replace("Train_GE3T", "Train")

    file_out = file_out.replace("T1", "scan")

    label_path = file_in.replace("T1", "wmh")
    flair_path = file_in.replace("T1", "FLAIR")

    source_label = nib.load(label_path)
    t1 = nib.load(file_in)
    flair = nib.load(flair_path)

    # Remap label 2.0 (pathologyd) to 0.0 (background)
    label_data = source_label.get_fdata()
    label_data[label_data == 2.0] = 0.0
    ...
    save_modified_label(file_out, modified_label=source_label)
...
def main(
    ...
    dataset_config = DatasetConfig(
        task_name="SEG008i_WMH_ISO",
        in_extensions=[".nii.gz"],
        n_classes=2,  # 0 = background, 1 = WMH, (2 = pathology -> remapped to 0)
        n_modalities=2,
        ...
        patterns_exclusion=[
            ...
            "wmh",
            "FLAIR",
            "3DT1",  # exclude for now, 3dslicer shows its a higher res version of T1
            # however label file is regular res
        ],
        patterns_DWI=[],
        patterns_PET=[],
        patterns_perfusion=[],
