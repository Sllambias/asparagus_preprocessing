...
def process(
    ...
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["derivatives", "sourcedata", "func", "bold", "fmap"],
        patterns_DWI=["dwi"],
        patterns_PET=[],
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"sub-([A-Z]\d+)"],
        df_columns=["participant_id", "session_id", "sex", "age", "handedness"],
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        # This file has been manually updated to include age at scan session
        # and merged for with mulitple files for more subjects
        demographics_csv_path = os.path.join(source_dir, "participants_updated.tsv")
