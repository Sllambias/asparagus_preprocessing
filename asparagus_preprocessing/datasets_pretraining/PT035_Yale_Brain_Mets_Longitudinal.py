...
def main(
    ...
    subdir: str = "Yale-Brain-Mets-Longitudinal",
    processes=12,
    bidsify=False,
...
def process(
    ...
    subdir: str = "Yale-Brain-Mets-Longitudinal",
    processes=12,
    task_name: str = "PT035_Yale_Brain_Mets_Longitudinal",
):
    dataset_config = DatasetConfig(
        ...
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"(YG_[A-Za-z0-9]{12})/([0-9]{4}-[0-9]{2}-[0-9]{2})"],
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        my_modality_overrides = {"_pre": {"suffix": "T1w", "folder": "anat"}}  # Pre contrast T1w
        # NOTE: MRI info has been created a priori with specific json file for each file from the xls file sheet!
        demographics_csv_path = os.path.join(source_dir, "Yale-Brain-Mets-Longitudinal-Clinical-Data.csv")
