...
def process(
    ...
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["derivatives", "func", "fmap", "VFA", "QMT", "phase_MEGRE"],
        patterns_DWI=["dwi"],
        patterns_PET=[],
        patterns_perfusion=["ASL", "m0scan"],
        patterns_m0=["m0scan"],
        patterns_bidsify=[r"sub-(\d{5})"],
        df_columns=["participant_id", "session_id", "age", "sex", "group", "ethnicity"],
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        demographics_csv_path = os.path.join(source_dir, "participants.tsv")
