...
def process(
    ...
    dataset_config = DatasetConfig(
        ...
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"sub-(control\d{6})", r"sub-(patient\d{6})"],
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        ...
        df = pd.read_csv(os.path.join(source_dir, "Taowu_version-1.0", "taowu", "taowu_patients.tsv"), delimiter="\t")
        demographics_csv_path = os.path.join(target_dir, "tmp.csv")
        df = df.drop("n_timepoints", axis=1)  # Getting pick up as session, but value is 229!
