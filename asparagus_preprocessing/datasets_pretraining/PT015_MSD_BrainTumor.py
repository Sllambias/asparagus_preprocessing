...
# Note that the original scans were a 4D image with T1, T1CE, T2, and FLAIR combined,
# Here I'm using the data after I separate the individual sequences
def main(
...
def process(
    ...
    subdir: str = "decathlon_Task1_Brain_Tumor_Split",
    processes=12,
    task_name: str = "PT015_MSD_BrainTumor",
):
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["labelsTr"],
        patterns_DWI=[],
        patterns_PET=[],
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"BraTS_(\d{3})"],
        df_columns=["participant_id", "session_id", "group"],
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        demographics_csv_path = ""  # No demographic file for this dataset
