...
def process(
    ...
    subdir: str = "Infant_Development_Brain",
    processes=12,
    task_name: str = "PT023_Infant_Development_Brain",
):
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["atlas"],
        patterns_DWI=[],
        patterns_PET=[],
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"(s\d{4})"],
        df_columns=["participant_id", "session_id", "age", "group"],
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        hardcoded_metadata = {
        # Only few info cause I cannot distinguish sedated vs non-sedated subjects (different protocol!)
            r"t1": {
                "Modality": "MR",
        ...
        df = pd.read_csv(os.path.join(source_dir, "meta.csv"))
        df = df.drop("group", axis=1)
        # Rough approximation: age from months to years (for 0 months, put 15 days)
        df["age"] = df["age"].apply(lambda m: 15 / 365 if m == 0 else m * 30 / 365)
