...
# Note that the original DTI were split as a single scan per bvalue,
# so I concatenate them together and call them DWI (a bit of an hack!)
def main(
...
def process(
    ...
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["DTI"],
        patterns_DWI=["DWI"],
        patterns_PET=[],
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"IXI(\d{3})"],
        df_columns=["participant_id", "session_id", "age", "sex", "group", "weight", "height", "occupation", "marital", "qualification"],
    )
    ...
    bvals_DWI, bvecs_DWI = get_bvals_and_bvecs_v2(
        files_DWI, os.path.join(source_dir, "bvals.txt"), os.path.join(source_dir, "bvecs.txt")
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        demographics_csv_path = os.path.join(source_dir, "IXI.xls")
        processed_files = recursive_find_files(target_dir, extensions=dataset_config.in_extensions + [".pt"])
        # No MRI info matching the actual files, skip it for now
