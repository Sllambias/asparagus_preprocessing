...
def create_cdr_groups(cdr_value):
    """
    Create group labels based on Clinical Dementia Rating (CDR) values.

    Args:
        cdr_value: CDR score

    Returns:
        str: Group label based on CDR score
    """
...
def main(
    ...
    subdir: str = "OASIS1/OASIS1_NIfTI",
    processes=12,
    bidsify=False,
...
def process(
    ...
    dataset_config = DatasetConfig(
        ...
        split=None,
        patterns_exclusion=["FSL_SEG", "PROCESSED"],
        patterns_DWI=[],
        patterns_PET=[],
        patterns_perfusion=[],
        patterns_m0=[],
        patterns_bidsify=[r"(OAS1_\d{4})_(MR\d{1})"],
        df_columns=["participant_id", "session_id", "sex", "age", "group", "handedness"],
    )
    ...
    if saving_config.bidsify or saving_config.save_dset_metadata:
        ...
        df = pd.read_excel(os.path.join(path, "OASIS1", "oasis_cross-sectional-5708aa0a98d82080.xlsx"))
        # Split the ID column into subject and session
        df[["subject", "session"]] = df["ID"].str.extract(r"(OAS1_\d{4})_(MR\d+)")
        df = df.drop("ID", axis=1)
        # Create group column based on CDR values
        df["group"] = df["CDR"].apply(create_cdr_groups)
