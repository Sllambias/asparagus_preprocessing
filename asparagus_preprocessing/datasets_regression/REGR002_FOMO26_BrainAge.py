...
def process_sample(file_in, file_out, dataset_config, preprocessing_config, saving_config):
    # Task 3 only has one modality: t1w.nii.gz.
    if os.path.basename(file_in) != "t1w.nii.gz":
        return
    ...
    label_path = file_in.replace("/preprocessed/", "/labels/").replace("t1w.nii.gz", "labels.txt")
